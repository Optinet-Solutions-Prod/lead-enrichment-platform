...
/** The built-in sources a recipe can include, with credit costs. */
const BUILT_IN_STEPS: StepOption[] = [
  ...
  { key: 'airbnb', label: 'Airbnb crawl (via Apify)', cost: 5, hint: 'Starts a ~300-listing browser crawl' },
]
...
export default async function PipelinePage() {
  ...
  const [{ data: recipeRows }, customDefs, balance] = await Promise.all([
    svc
      .from('org_recipes')
  ...
  const recipes: RecipeView[] = ((recipeRows ?? []) as Array<{
    ...
    cost: r.steps.sources.reduce(
      (sum, s) => sum + (s === 'airbnb' ? 5 : 1),
      0,
    ),
    lastRun: r.last_run,
  }))
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-6 p-4">
      <PageIntro
        ...
        statsLine={`${recipes.length} saved · ${balance.toLocaleString()} credits available`}
        relations={[
      ...
      <RecipeBuilder steps={stepOptions} />
