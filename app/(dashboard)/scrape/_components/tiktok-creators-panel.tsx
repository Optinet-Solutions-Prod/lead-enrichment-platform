...
import { Music2, CheckCircle2, Loader2, Play, Sparkles } from 'lucide-react'
import {
  runTiktokProfileEnrichment,
  runTiktokCreatorAnalysis,
  type StageRunState,
} from '../actions'
import type { TiktokCreatorSummary } from '../_lib/queries'

const initialState: StageRunState = null

/**
 * TikTok creator panel (Phases 2 + 3). Shown only for search_engine='tiktok'
 ...
 */
export function TiktokCreatorsPanel({
  jobId,
  summary,
}: {
  jobId: string
  summary: TiktokCreatorSummary
}) {
  ...
  const enrichTitle = noCreators
    ? 'No creators discovered yet — run the TikTok scrape first'
    : inflight
      ? 'Profile enrichment already running — wait for it to finish'
      : nothingToEnrich
        ? 'Every discovered creator already enriched'
        : 'Enrich the discovered profiles (followers, bio, bio link, recent captions)'
    ...
    <section className="rounded-lg border border-[color:var(--color-border)] bg-[color:var(--color-bg-secondary)] p-3">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span
          ...
          title="Operator-triggered — click ▶ to render each tiktok.com/@{handle} profile"
        >
          manual
        </span>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <form action={enrichAction}>
            ...
            <button
              ...
              aria-label="Enrich TikTok creator profiles"
              title={enrichTitle}
