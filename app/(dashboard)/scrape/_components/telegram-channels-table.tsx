 ...
 * count, contacts (email › Telegram › Discord, mined from title/description),
 * and the casino links the channel posts (the affiliate funnel). Server component.
 *
 * Sibling of snapchat-creators-table.tsx.
 */
export function TelegramChannelsTable({ rows }: { rows: TelegramChannelRow[] }) {
        ...
        <thead className="bg-[color:var(--color-bg-secondary)]">
          <tr className="text-left text-[11px] text-[color:var(--color-text-secondary)] [&>th]:sticky [&>th]:top-0 [&>th]:z-20 [&>th]:border-b [&>th]:border-[color:var(--color-border)] [&>th]:bg-[color:var(--color-bg-secondary)]">
            ...
            <th
              ...
              title="Affiliate likelihood + niche score (0–100). “affiliate” = scored ≥30 (or posts a casino link directly). NEW = a likely affiliate flagged as a new lead candidate. Hover a badge for the breakdown."
            >
              Affiliate
            </th>
            <th
              className="cursor-help px-3 py-2 font-medium"
              title="Outreach contacts mined from the title + description — priority email › Telegram › Discord. Run “Score & check” to populate."
            >
              Contact
            </th>
            <th className="px-3 py-2 font-medium text-right">Subscribers</th>
            <th className="px-3 py-2 font-medium">Casino partners / posted links</th>
          </tr>
        </thead>
        <tbody>
          {rows.map(r => (
            <TelegramChannelRowView key={r.id} r={r} />
          ))}
        </tbody>
      </table>
    </div>
...
function TelegramChannelRowView({ r }: { r: TelegramChannelRow }) {
  ...
  return (
    <tr className="border-b border-[color:var(--color-border)] last:border-0 align-top">
      <td className="px-3 py-2">
        ...
        >
          <Send className="h-3 w-3 text-sky-500 shrink-0" />
          {r.title || `@${r.username}`}
        </a>
          ...
          <div className="flex flex-wrap items-center gap-1">
            {r.is_likely_affiliate ? (
              <span
                ...
                title={`Likely casino affiliate. Niche score ${niche}/100 (≥30 is flagged). Built from the casino links the channel posts, gambling keywords in the title/description, and a gambling-flavoured name — higher = stronger signal.`}
              >
...
/** Outreach contacts: email › Telegram › Discord, mined from title/description. */
function ContactCell({ r }: { r: TelegramChannelRow }) {
