...
import { setCaptchaSolverEnabledAction, type SettingState } from '../actions'

const initial: SettingState = null

export function CaptchaSolverToggle({ enabled }: { enabled: boolean }) {
  const [state, action, pending] = useActionState(setCaptchaSolverEnabledAction, initial)
    ...
    <div className="flex flex-col gap-2">
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          />
          Captcha solver is currently {enabled ? 'ON' : 'OFF'}
        </span>
        ...
        <form action={action}>
          ...
          <button
            ...
            title={
              enabled
                ? 'Turn off Captcha solver — captchas will fail the job instantly'
                : 'Turn on Captcha solver — captchas will park in /admin/interactive'
            }
          >
            {pending ? (
            ...
            {enabled ? 'Turn Captcha solver OFF' : 'Turn Captcha solver ON'}
          </button>
        </form>
      </div>
