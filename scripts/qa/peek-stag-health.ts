...
async function main() {
  ...
  const { data: qRows, error: qe } = await svc
    ...
    .limit(2000)
  if (qe) { console.error('queue err', qe); process.exit(1) }

  const byStatus: Record<string, number> = {}
  for (const r of qRows ?? []) byStatus[r.status] = (byStatus[r.status] ?? 0) + 1
  console.log(`\n=== S-tag queue rows updated in last 7d: ${qRows?.length ?? 0} ===`)
  console.log('  by status:', byStatus)

  // 2. Tags inserted in last 7d.
  const { data: tagRows, error: te } = await svc
    .from('s_tags_table')
    .select('id, lead_id, extracted_via, created_at')
    .gte('created_at', since)
  if (te) { console.error('tags err', te); process.exit(1) }
  ...
  console.log(`\n=== s_tags_table inserts in last 7d: ${tagRows?.length ?? 0} rows across ${distinctLeads} leads ===`)
  ...
  // 4. Recently-checked leads with zero tags.
  const { data: checkedLeads, error: ce } = await svc
    .from('google_lead_gen_table')
    .select('id, url, s_tags_checked_at')
    .gte('s_tags_checked_at', since)
    .order('s_tags_checked_at', { ascending: false })
    .limit(500)
  if (ce) { console.error('leads err', ce); process.exit(1) }

  const leadIds = (checkedLeads ?? []).map(l => l.id)
  const haveTags = new Set<number>()
  for (let i = 0; i < leadIds.length; i += 50) {
    ...
    const { data: tt } = await svc
      ...
      .select('lead_id')
      .in('lead_id', slice)
    for (const r of tt ?? []) haveTags.add(r.lead_id as number)
  }
  const zeroTagLeads = (checkedLeads ?? []).filter(l => !haveTags.has(l.id))
  console.log(`\n=== Leads with s_tags_checked_at in 7d: ${checkedLeads?.length ?? 0}, of which 0 tags landed: ${zeroTagLeads.length} ===`)
